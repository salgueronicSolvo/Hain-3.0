...
namespace AnythingWorld.Editor
{
    public class MyWorldEditor : AnythingCreatorEditor
    {
        ...
        private List<SearchResult> collectionObjects;
        #endregion Fields
        ...
        protected new void Awake()
        {
            ...
            if(AnythingSettings.Instance.HasAPIKey()) InitializeCollectionsAndLikes();
        ...
        protected new void OnGUI()
        {
            ...
            if (Event.current.type == EventType.Repaint && !AnythingSettings.Instance.HasAPIKey()) Close();
        ...
        private void DrawMyWorld()
        {
            ...
            if (GUILayout.Button(new GUIContent("My Likes", myWorldTabCategory == MyWorldTabCategory.LIKES ? StateLikesIcon.activeTexture : StateLikesIcon.inactiveTexture), myWorldTabCategory == MyWorldTabCategory.LIKES ? TabButtonActiveStyle : TabButtonInactiveStyle, GUILayout.MaxWidth(position.width / 2)))
            {
                ...
                //searchMode = likedObjects == null || likedObjects.Count == 0 ? SearchMode.RUNNING : SearchMode.RUNNING_SILENTLY;
                searchMode = SearchMode.RUNNING;
        ...
        private void DrawListCard(List<CollectionResult> resultArray, float columnCoord, float rowCoord, float buttonWidth, float buttonHeight, int searchIndex, float scaleMultiplier)
        {
            ...
            FindCategoryIcon(CreationSearchCategory.COLLECTION, searchIndex, out var categoryIcon);
            DrawCategoryIcon(thumbnailBackdropRect, infoPaddingX, infoPaddingY, categoryIcon, scaleMultiplier);
        }
        ...
        protected void DrawListItemCard(List<SearchResult> resultArray, float columnCoord, float rowCoord, float buttonWidth, float buttonHeight, int searchIndex, float scaleMultiplier)
        {
            ...
            DrawVoteCountLabel(infoPaddingX, voteRect, result.data.voteScore, scaleMultiplier);
            DrawDeleteListIcon(result, infoBackdropRect, infoPaddingX, infoPaddingY, scaleMultiplier);

            DrawAuthorIconBackground(ref infoBackdropRect, ref thumbnailBackdropRect, infoPaddingX, infoPaddingY, out var authorIconRect, scaleMultiplier);
            DrawModelNameLabel(modelName, infoPaddingX, cardRect.xMax, ref authorIconRect, out var modelNameLabelRect, scaleMultiplier);
            DrawAuthorLabel(authorName, infoPaddingX, cardRect.xMax, authorIconRect, modelNameLabelRect, scaleMultiplier);

            DrawAnimationStatusIcon(thumbnailBackdropRect, infoPaddingX, infoPaddingY, scaleMultiplier);
        ...
        protected void DrawDeleteCollectionWindow(AnythingEditor window)
        {
            ...
            {
                CollectionProcessor.DeleteCollection(currentCollection.Name, this);
                modelLevel = false;
                currentCollection = null;
                searchMode = SearchMode.RUNNING;
                CollectionProcessor.GetCollections(RefreshCollectionResults, this);
        ...
        protected void DrawRemoveFromCollectionWindow(AnythingEditor window, SearchResult result)
        {
            ...
            {
                CollectionProcessor.RemoveFromCollection(result, currentCollection.Name, this);
                modelLevel = true;
                searchMode = SearchMode.RUNNING;
        ...
        public void RefreshLikedObjects(List<SearchResult> results)
        {
            if (likedObjects == null && results == null)
            {
                searchMode = SearchMode.FAILURE;
