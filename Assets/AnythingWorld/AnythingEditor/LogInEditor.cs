    ...
    public class LogInEditor : AnythingCreatorEditor
    {
        ...
        private void DrawLogin()
        {
            ...
            else
            {
                ...
                {
                    if (loginTabCategory == LoginTabCategory.LOGIN) SignupLoginProcessor.LogIn(loginEmailField, loginPasswordField, LoginErrorHandler, LoginSuccessHandler, this);
                    else if (loginTabCategory == LoginTabCategory.SIGNUP) SignupLoginProcessor.SignUp(signupEmailField, signupPasswordField, signupPasswordCheckField, signupFullNameField, signupTermsAccepted, SignupErrorHandler, LoginSuccessHandler, this);
                }
            }
        }
        ...
        private void LoginErrorHandler(SignupLoginProcessor.LoginError loginError)
        {
            switch (loginError)
            {
                case SignupLoginProcessor.LoginError.InvalidFormat:
                    loginErrorMessage = "Invalid password or email.";
                    break;
                case SignupLoginProcessor.LoginError.ServerError:
                    loginErrorMessage = "Server connection problem, please try again.";
                    break;
                case SignupLoginProcessor.LoginError.EmptyField:
                    loginErrorMessage = "All fields must be filled.";
                    break;
                case SignupLoginProcessor.LoginError.UnregisteredEmail:
                    loginErrorMessage = "No account with this email found.";
                    break;
                default:
                    break;
            }
        ...
        private void SignupErrorHandler(SignupLoginProcessor.SignupError signupError)
        {
            switch (signupError)
            {
                case SignupLoginProcessor.SignupError.InvalidPassword:
                    loginErrorMessage = "Your password must include at least 1 letter and 1 number.";
                    break;
                case SignupLoginProcessor.SignupError.PasswordLength:
                    loginErrorMessage = "Your password needs to be at least 12 characters long.";
                    break;
                case SignupLoginProcessor.SignupError.PasswordMatch:
                    loginErrorMessage = "Your passwords don't match.";
                    break;
                case SignupLoginProcessor.SignupError.InvalidEmail:
                    loginErrorMessage = "Your email is an invalid address.";
                    break;
                case SignupLoginProcessor.SignupError.RegisteredEmail:
                    loginErrorMessage = "Your email is already registered.";
                    break;
                case SignupLoginProcessor.SignupError.EmptyField:
                    loginErrorMessage = "All fields must be filled.";
                    break;
                case SignupLoginProcessor.SignupError.EmptyPassword:
                    loginErrorMessage = "All fields must be filled.";
                    break;
                case SignupLoginProcessor.SignupError.EmptyEmail:
                    loginErrorMessage = "All fields must be filled.";
                    break;
                case SignupLoginProcessor.SignupError.ServerError:
                    loginErrorMessage = "Server connection problem, please try again.";
                    break;
                case SignupLoginProcessor.SignupError.Terms:
                    loginErrorMessage = "To create an account, you must accept our terms and conditions.";
                    break;
                default:
                    break;
            }
        ...
        {
            loginErrorMessage = "";
            CoroutineExtension.StartEditorCoroutine(WaitThen(1f, PresentSuccess, RestartAnythingWorld), this);
        }
        ...
        //?: Perhaps we make this a generic function to call from anywhere, perhaps in CoroutineExtension?
        private IEnumerator WaitThen(float length, Action firstDo, Action whenDone)
        {
            firstDo.Invoke();
            yield return new EditorWaitForSeconds(length);
            whenDone.Invoke();
        }
